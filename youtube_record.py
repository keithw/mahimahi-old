...
def main():
	...
	if os.path.exists(saved_requests_path): 
		should_run_mm_webrecord = get_yes_or_no("It appears as if you have already recorded request \
			response pairs for this YouTube url. Would you like to overwrite your previous recording? y/n")
		if(should_run_mm_webrecord):
			print "RUNNING mm-webrecord to record new session. Please play video when browser pops up and wait \
			20 seconds to save enough requests."
	...
	if os.path.exists(media_files_path):
		should_run_youtube_dl = get_yes_or_no("It appears you have already downloaded this youtube video locally. \
			Would you like to overwrite your previous download and re-download this youtube video? y/n")
