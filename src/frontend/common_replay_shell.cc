...
CommonReplayShell::CommonReplayShell (bool isYoutubeServer) : isYoutubeServer(isYoutubeServer) {
    
}

void CommonReplayShell::add_dummy_interface( const string & name, const Address & addr )
{
...
int CommonReplayShell::run_replay_shell( int argc, char *argv[] )
{
    try {
        /* clear environment */
        ...
        for ( const auto ip_port : unique_ip_and_port ) {
            servers.emplace_back( ip_port, working_directory, directory , isYoutubeServer);
        }
